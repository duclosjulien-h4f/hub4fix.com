...
Aucun lien entrant, aucun rôle dans le site déployé. ~1,2 Mo.
...
| `hub4fix.com/jarvis-demo.html` | 40 Ko | Se décrit lui-même en en-tête : *« Maquette jetable : tout est inline pour ne rien impacter du site réel. »* `jarvis.html` (la vraie page) ne la cite que dans un **commentaire CSS**, pas dans un lien — la retirer ne casse aucune navigation. |
...
git rm "logo hub4fix_v1_2025-Sep-07_03-01-05PM-000_CustomizedView22696223382.png" \
       ...
       "logo H4F.png" \
       hub4fix.com/jarvis-demo.html
git rm -r demo-hub4fix
```

Note : la ligne « Repris à l'identique de la maquette jarvis-demo.html » dans
`hub4fix.com/jarvis.html:11` devient un renvoi mort — commentaire à ajuster.
...
| `hub4fix.com/jarvis.html` | Vraie page assistant, `noindex` en attendant l'ouverture publique (voir son en-tête). Charge `js/account-menu-b2c.js`. |
...
1. **Référence cassée.** `hub4fix.com/images/favicon-h4f.svg` n'existe pas, alors que
   ...
   `js/`, `data/`). Si la catégorie A est appliquée, il ne reste que `jarvis.html`
   à corriger — vers `favicon.svg`.
...
|---|---|---|---|
| A — sans risque | 5 (dont 1 dossier) | ~1,2 Mo | supprimable tel quel |
| B — ancienne démo | 3 | 72 Ko | **corriger `js/nav-mindmap.js:103` d'abord** |
| C — à arbitrer | 4 | ~18 Ko | décision produit requise |
